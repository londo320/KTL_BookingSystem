  ...
  @foreach($bookings->groupBy(fn($b) => $b->slot?->depot?->name ?? 'Unknown Depot') as $depotName => $group)
    ...
    @foreach($group->sortBy(fn($b) => $b->slot?->start_at ?? now()) as $booking)
      ...
      <tr class="border-t hover:bg-gray-50 
        @if($isFactory) bg-purple-50 border-l-4 border-purple-400
        @elseif($booking->cancelled_at) bg-red-50 border-red-200 
        ...
        <td class="px-4 py-2 align-top">
          <div class="flex items-center">
            ...
            @endif
            <div class="font-mono text-sm font-semibold {{ $isFactory ? 'text-purple-600' : 'text-blue-600' }}">
              @if($isFactory)
        ...
        {{-- Start → End with live Late timer --}}
                ...
                <div class="text-sm">
                  @if($arrivedAt)
                    <div class="text-gray-700">
                      Arrived: {{ $arrivedAt->format('d-M H:i') }}
                    </div>
                    @if(!$completedAt)
                      <div class="{{ $isOverdue ? 'text-red-600 font-semibold' : 'text-blue-600' }} text-xs">
                        On site: {{ floor($minutesOnSite / 60) }}h {{ $minutesOnSite % 60 }}m
                        @if($isOverdue)
                    ...
                    @else
                      <div class="text-green-600 text-xs">
                        Completed: {{ $completedAt->format('d-M H:i') }}
                      </div>
                    @endif
                  @else
              ...
              @else
                {{-- Regular booking slot timing logic --}}
                @php
        ...
        {{-- Type & PO Numbers --}}
        <td class="px-4 py-2 align-top">
          <div class="text-sm font-medium {{ $isFactory ? 'text-purple-600' : '' }}">{{ optional($booking->bookingType)->name ?? '-' }}</div>
        ...
        <td class="px-4 py-2 align-top space-y-1">
          ...
          @else
            ...
            @if($isFactory && $booking->trailer_registration)
              <div class="text-xs text-gray-600">🚚 {{ $booking->trailer_registration }}</div>
            @endif
