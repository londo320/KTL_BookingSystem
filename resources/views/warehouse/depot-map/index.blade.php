<x-warehouse-layout>
        ...
        <div class="mb-6 grid grid-cols-2 md:grid-cols-6 gap-4">
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
                ...
                <div class="text-xs text-gray-500 mt-1 space-y-1">
                    <div>Expected Units: {{ number_format($activitySummary['expected_units']) }}</div>
                    <div>Actual Units: {{ number_format($activitySummary['actual_units']) }}</div>
                    <div>Expected Pallets: {{ number_format($activitySummary['expected_pallets']) }}</div>
                    <div>Actual Pallets: {{ number_format($activitySummary['actual_pallets']) }}</div>
                </div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-teal-500">
                ...
                <div class="text-xs text-gray-500 mt-1 space-y-1">
                    <div>Units: {{ number_format($activitySummary['today_processed_units']) }}</div>
                    <div>Pallets: {{ number_format($activitySummary['today_processed_pallets']) }}</div>
                </div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
                <div class="text-sm text-gray-600">Capacity</div>
                <div class="text-2xl font-bold text-green-600" id="available-count">{{ $activitySummary['available_capacity'] }}</div>
                <div class="text-xs text-gray-500 mt-1">
                    Total: {{ $activitySummary['total_capacity'] }} | In Use: {{ $activitySummary['in_use_capacity'] }} | Available: {{ $activitySummary['available_capacity'] }}
                </div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-yellow-500">
                <div class="text-sm text-gray-600">Full Trailers</div>
                <div class="text-2xl font-bold text-yellow-600" id="full-count">{{ $activitySummary['total_full_trailers'] }}</div>
                <div class="text-xs text-gray-500 mt-1">
                    Parking Zone: {{ $activitySummary['full_in_parking'] }} | Tipping: {{ $activitySummary['full_currently_tipping'] }}
                </div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-orange-500">
                ...
                <div class="text-xs text-gray-500 mt-1">
                    Parking Area: {{ $activitySummary['awaiting_in_parking'] }} | At Bay: {{ $activitySummary['awaiting_in_bays'] }}
                </div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
                ...
                <div class="text-xs text-gray-500 mt-1">
                    Arrived: {{ $activitySummary['todays_arrivals'] }} | Pending: {{ $activitySummary['pending_arrivals'] }}
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        async function showLocationDetails(locationId) {
            ...
            try {
                const url = `{{ url('app/depot-map/location') }}/${locationId}?v=${Date.now()}`;
