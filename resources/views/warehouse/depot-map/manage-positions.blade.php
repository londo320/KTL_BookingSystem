<x-warehouse-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-xl text-gray-800">🎯 Manage Item Positions - {{ $depot->name }}</h2>
                                    ...
                                    <div class="relative max-w-full max-h-full" id="map-image-container">
                                        ...
                                        @foreach($bays as $bay)
                                            <div class="absolute bay-marker cursor-move" 
                                                 ...
                                                 style="top: {{ $bay->map_y ?? 50 }}%; left: {{ $bay->map_x ?? 50 }}%; {{ !$bay->show_on_map ? 'display: none;' : '' }}"
                                                 title="Drag to reposition {{ $bay->name }}">
                                                @php
                                                ...
                                                @endphp
                                                <div class="bay-box" 
                                                     style="width: {{ $bayWidth }}px; height: {{ $bayHeight }}px; 
                                                            background: rgba(59, 130, 246, 0.8); 
                                                            border: 2px solid #2563eb; 
                                                            border-radius: 4px;
                                                            transform: rotate({{ $bayRotation }}deg);
                                                            display: flex;
                                                            flex-direction: column;
                                                            justify-content: center;
                                                            align-items: center;">
                                                    <div class="text-{{ $bayTextSize }} font-bold text-center" style="color: {{ $bayTextColor }};">
                                        ...
                                        @foreach($locations as $location)
                                            ...
                                            <div class="absolute location-marker cursor-move" 
                                                 ...
                                                 style="top: {{ $location->map_y ?? 30 }}%; left: {{ $location->map_x ?? 30 }}%; {{ !$location->show_on_map ? 'display: none;' : '' }}"
                                                 title="Drag to reposition {{ $location->name }}">
                                                <div class="location-box" 
                                                     style="width: {{ $locationWidth }}px; height: {{ $locationHeight }}px; 
                                                            ...
                                                            justify-content: center;
                                                            align-items: center;
                                                            transform: rotate({{ $locationRotation }}deg);">
                                                    <div class="text-sm mb-1">{{ $icon }}</div>
                                                    <div class="text-{{ $locationTextSize }} font-bold text-center leading-tight" style="color: {{ $locationTextColor }};">
                                                        {{ $location->code ?? Str::limit($location->name, 8) }}
                                                    </div>
                                                    <div class="text-xs text-center" style="color: {{ $locationTextColor }};">
                                                        {{ $location->capacity }}
                                                    </div>
                                                </div>
                                                <!-- Drag handle -->
            ...
            <div class="xl:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow">
                    <div class="p-4 border-b border-gray-200">
                        ...
                        <p class="text-xs text-gray-500 mt-1">{{ $bays->count() }} bays, {{ $locations->count() }} locations for {{ $depot->name }}</p>
                    </div>
                                ...
                                <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
                                    <div class="flex items-center space-x-3">
                                        <input type="checkbox" 
                                               ...
                                               data-bay-id="{{ $bay->id }}"
                                               {{ $bay->show_on_map ? 'checked' : '' }}>
                                        <div>
                                            <div class="font-medium text-sm">{{ $bay->name }}</div>
                                            @if($bay->code)
                                                <div class="text-xs text-gray-500">{{ $bay->code }}</div>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="text-xs text-gray-400">
                                        @if($bay->map_x && $bay->map_y)
                                            {{ number_format($bay->map_x, 1) }}%, {{ number_format($bay->map_y, 1) }}%
                                        @else
                                            Not positioned
                                        @endif
                                    </div>
                                        ...
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <div>
                                                ...
                                                <div class="text-xs text-gray-500">
                                                    {{ $location->code ? $location->code . ' • ' : '' }}Capacity: {{ $location->capacity }}
                                                </div>
                                            </div>
                                        </div>
                                        <div class="text-xs text-gray-400">
                                            @if($location->map_x && $location->map_y)
                                                {{ number_format($location->map_x, 1) }}%, {{ number_format($location->map_y, 1) }}%
                                            @else
                                                Not positioned
                                            @endif
                                        </div>
