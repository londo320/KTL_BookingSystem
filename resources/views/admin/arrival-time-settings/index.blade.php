...
@section('title', 'Arrival Time Settings')
...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                ...
                <div>
                    ...
                    <select name="depot_id" id="depot_id" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        @foreach($depots as $depot)
                            <option value="{{ $depot->id }}" {{ $depotId == $depot->id ? 'selected' : '' }}>{{ $depot->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
    ...
    <div class="bg-white rounded-lg shadow-sm border">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">⚙️ Current Settings ({{ $settings->total() }} total)</h3>
                        ...
                        <tr class="hover:bg-gray-50">
                                ...
                                <div class="flex items-center">
                                    <div>
                                        ...
                                        <div class="text-sm text-gray-900">
                                            ...
                                            @elseif($setting->level === 'depot')
                                                🏢 {{ $setting->depot->name ?? 'Unknown Depot' }}
                                            @elseif($setting->level === 'customer')
                                                👤 {{ $setting->customer->name ?? 'Unknown Customer' }}
                                                @if($setting->customer && $setting->customer->depot)
                                                    <br><span class="text-xs text-gray-500">@ {{ $setting->customer->depot->name }}</span>
                                                @endif
                                            @endif
                                        </div>
                                    </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                                    ⏪ {{ $setting->early_threshold_minutes }}min
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="text-xs text-gray-500 mt-1">
                                    On-time window: {{ $setting->early_threshold_minutes }}min early to {{ $setting->late_threshold_minutes }}min late
                                </div>
                            </td>
