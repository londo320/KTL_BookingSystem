...
<div class="py-6 max-w-7xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm border">
        <div class="px-6 py-4 border-b border-gray-200">
            <h1 class="text-2xl font-semibold text-gray-900">Edit Custom Role</h1>
            <p class="text-sm text-gray-600 mt-1">{{ $customRole->display_name }} ({{ $customRole->users()->count() }} users assigned)</p>
        </div>

        <form method="POST" action="{{ route('app.custom-roles.update', $customRole) }}" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="name" name="name" required
                                   value="{{ old('name', $customRole->name) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="display_name" name="display_name" required
                                   value="{{ old('display_name', $customRole->display_name) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" name="description" rows="3"
                                      ...
                                      class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500">{{ old('description', $customRole->description) }}</textarea>
                        ...
                        <div class="mb-4">
                            <label class="flex items-center">
                                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $customRole->is_active) ? 'checked' : '' }}
                ...
                <div class="lg:col-span-2">
                    ...
                    {{-- Function Categories --}}
                    @php
                        $currentFunctionKeys = old('function_keys', $customRole->getFunctionKeys());
                    @endphp
                                    ...
                                    @foreach($functions as $key => $label)
                                        <label class="flex items-start cursor-pointer hover:bg-white rounded p-1 -m-1">
                                            <input type="checkbox"
                                                   ...
                                                   data-category="{{ Str::slug($category) }}"
                                                   {{ in_array($key, $currentFunctionKeys) ? 'checked' : '' }}
