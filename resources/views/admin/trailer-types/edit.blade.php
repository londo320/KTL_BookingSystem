...
@section('title', 'Edit Trailer Type')
        ...
        <div class="flex">
            ...
            <div class="ml-3">
                <p class="text-sm text-amber-800">
                    🔒 <strong>Protected Data:</strong> This trailer type has {{ $trailerType->bookings()->count() }} associated bookings and cannot be deleted, only deactivated.
                </p>
            </div>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           name="name" 
                           id="name" 
                           value="{{ old('name', $trailerType->name) }}"
                           required
                ...
                <div>
                    ...
                    <textarea name="description" 
                              ...
                              placeholder="Optional description of this trailer type...">{{ old('description', $trailerType->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-sm text-gray-500">
                    ...
                    <div class="mt-2">
                        <label class="inline-flex items-center">
                            <input type="checkbox" 
                                   name="is_active" 
                                   value="1" 
                                   {{ old('is_active', $trailerType->is_active) ? 'checked' : '' }}
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600">{{ $trailerType->created_at->format('M j, Y g:i A') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Last Updated</label>
                        <p class="mt-1 text-sm text-gray-600">{{ $trailerType->updated_at->format('M j, Y g:i A') }}</p>
                    </div>
                </div>
            </div>
