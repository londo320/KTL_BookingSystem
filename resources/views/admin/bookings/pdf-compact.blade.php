...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Admin Report | Generated: {{ now()->format('d M Y, H:i') }}</p>
        </div>
        ...
        <div class="two-column">
            <div class="column">
                ...
                <div class="section">
                    ...
                    @if($booking->total_expected_cases || $booking->total_actual_cases)
                        <div class="load-row">
                            <div class="load-expected">
                                <strong>Cases:</strong> {{ number_format($booking->total_expected_cases ?? 0) }}
                            </div>
                            @if($booking->total_actual_cases)
                                <div class="load-arrow">→</div>
                                <div class="load-actual">
                                    <strong>{{ number_format($booking->total_actual_cases) }}</strong>
                                    @php $caseDiff = $booking->total_actual_cases - ($booking->total_expected_cases ?? 0); @endphp
                                    @if($caseDiff != 0)
                    ...
                    @if($booking->total_expected_pallets || $booking->total_actual_pallets)
                        <div class="load-row">
                            <div class="load-expected">
                                <strong>Pallets:</strong> {{ number_format($booking->total_expected_pallets ?? 0) }}
                            </div>
                            @if($booking->total_actual_pallets)
                                <div class="load-arrow">→</div>
                                <div class="load-actual">
                                    <strong>{{ number_format($booking->total_actual_pallets) }}</strong>
                                    @php $palletDiff = $booking->total_actual_pallets - ($booking->total_expected_pallets ?? 0); @endphp
                                    @if($palletDiff != 0)
