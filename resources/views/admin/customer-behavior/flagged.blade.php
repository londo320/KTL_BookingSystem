...
@section('title', 'Flagged Customers')
...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="bg-white rounded-lg shadow-sm border p-6 text-center">
            <div class="text-3xl font-bold text-red-600">{{ $flaggedCustomers->count() }}</div>
            <div class="text-sm text-gray-600 mt-1">🚩 Flagged Customers</div>
        </div>
        <div class="bg-white rounded-lg shadow-sm border p-6 text-center">
            <div class="text-3xl font-bold text-amber-600">{{ $flaggedCustomers->where('last_minute_count', '>=', 5)->count() }}</div>
            <div class="text-sm text-gray-600 mt-1">⏰ High Risk</div>
        </div>
        <div class="bg-white rounded-lg shadow-sm border p-6 text-center">
            <div class="text-3xl font-bold text-blue-600">{{ $flaggedCustomers->sum('rebook_count') }}</div>
            <div class="text-sm text-gray-600 mt-1">🔄 Total Rebooks</div>
        </div>
        <div class="bg-white rounded-lg shadow-sm border p-6 text-center">
            <div class="text-3xl font-bold text-cyan-600">{{ $flaggedCustomers->sum('cancellation_count') }}</div>
    ...
    <div class="bg-white rounded-lg shadow-sm border">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">⚠️ High Risk Customers ({{ $flaggedCustomers->count() }} found)</h3>
            <p class="text-sm text-gray-600 mt-1">Customers requiring management attention and possible intervention</p>
        </div>
        <div class="overflow-hidden">
            @if($flaggedCustomers->count() > 0)
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 {{ $customer->last_minute_count >= 5 ? 'bg-red-50 border-l-4 border-red-500' : 'bg-amber-50 border-l-4 border-amber-500' }}">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-wrap gap-1">
                                    @if($customer->last_minute_count >= 5)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                            ⏰ Excessive Last-Minute
                                        </span>
                                    @elseif($customer->last_minute_count >= 3)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">
                                            ⏰ Frequent Last-Minute
                                        </span>
                                    @endif
                                    
                                    @if($customer->rebook_count >= 10)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                            🔄 Excessive Rebooks
                                        </span>
                                    @elseif($customer->rebook_count >= 5)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">
                                            🔄 Frequent Rebooks
                                        </span>
                                    @endif
                                    
                                    @if($customer->cancellation_count >= 10)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                            ❌ High Cancellations
                                        </span>
                                    @elseif($customer->cancellation_count >= 5)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $customer->last_minute_count >= 5 ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800' }}">
                                    ⏰ {{ $customer->last_minute_count }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $customer->rebook_count >= 10 ? 'bg-red-100 text-red-800' : 'bg-amber-100 text-amber-800' }}">
                                ...
                                @if($customer->last_minute_dates)
                                    <div class="flex flex-wrap gap-1">
                                        ...
                                        @foreach($dates as $date)
                                            ...
                                            @endif
                                        @endforeach
                                        @if(count(explode(',', $customer->last_minute_dates)) > 3)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <button type="button" class="inline-flex items-center px-3 py-1 border border-amber-300 text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-md text-sm font-medium transition-colors"
                                            title="Contact Customer" onclick="contactCustomer('{{ $customer->email ?? '' }}', '{{ $customer->name }}')">
                                        📧 Contact
                                    </button>
                                    <button type="button" class="inline-flex items-center px-3 py-1 border border-red-300 text-red-700 bg-red-50 hover:bg-red-100 rounded-md text-sm font-medium transition-colors"
                                            title="Restrict Customer" onclick="restrictCustomer({{ $customer->id }}, '{{ $customer->name }}')">
                                        🚫 Restrict
                                    </button>
                                </div>
                            </td>
