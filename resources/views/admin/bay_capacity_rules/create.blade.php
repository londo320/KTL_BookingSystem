...
<div class="container mx-auto px-4 py-6 max-w-3xl">
    ...
    <form action="{{ route('app.bay-capacity-rules.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="bg-white shadow rounded-lg p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="time" name="time_start" value="{{ old('time_start', '08:00') }}" required
                           class="block w-full border-gray-300 rounded text-sm py-2">
                    @error('time_start')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="time" name="time_end" value="{{ old('time_end', '17:00') }}" required
                           class="block w-full border-gray-300 rounded text-sm py-2">
                    @error('time_end')
                        <p class="text-red-600 text-xs mt-1">{{ $message }}</p>
                    @enderror
                    ...
                    @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
                        <label class="flex items-center text-sm">
                            <input type="checkbox" name="days_of_week[]" value="{{ $day }}"
                                   @checked(in_array($day, old('days_of_week', [])))
                                   class="mr-1">
                            {{ substr($day, 0, 3) }}
                        </label>
                    @endforeach
            ...
            <div>
                ...
                <input type="number" name="max_concurrent_bookings" value="{{ old('max_concurrent_bookings', 3) }}"
            ...
            <div>
                ...
                <input type="number" name="capacity_weight" value="{{ old('capacity_weight', 1.0) }}"
