<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800">Edit Equipment Type</h2>
  </x-slot>

  <div class="py-6 max-w-4xl mx-auto">
    <form method="POST" action="{{ route('app.equipment-types.update', $equipmentType) }}">
      @csrf
      @method('PUT')
      ...
      <div class="bg-white p-6 rounded shadow space-y-4">
        <div>
          ...
          <input type="text" name="name" class="w-full border rounded p-2" value="{{ old('name', $equipmentType->name) }}" required>
        ...
        <div>
          ...
          <input type="text" name="key" class="w-full border rounded p-2 font-mono" value="{{ old('key', $equipmentType->key) }}" required pattern="[a-z0-9_]+">
        ...
        <div>
          ...
          <textarea name="description" class="w-full border rounded p-2" rows="3">{{ old('description', $equipmentType->description) }}</textarea>
        ...
        <div>
          ...
          <input type="number" name="sort_order" class="w-full border rounded p-2" value="{{ old('sort_order', $equipmentType->sort_order) }}" required min="0">
        ...
        <div>
          <label class="flex items-center">
            <input type="checkbox" name="is_active" value="1" class="mr-2" @checked(old('is_active', $equipmentType->is_active))>
