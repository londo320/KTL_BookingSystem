        ...
        <div class="mb-6 p-4 bg-green-100 border border-green-400 text-green-700 rounded">
            ...
            @if(session('command_output'))
                <details class="mt-2">
                    ...
                    <pre class="mt-2 p-2 bg-green-50 rounded text-xs overflow-auto">{{ session('command_output') }}</pre>
                </details>
            @endif
        </div>
    @endif

    <form action="{{ route('app.bay-slot-generation.generate') }}" method="POST" id="slot-generation-form" class="space-y-6">
        @csrf
        ...
        <div class="bg-white shadow rounded-lg p-6 space-y-4">
            ...
            <div>
                ...
                <select name="depot_id" id="depot-select" required class="block w-full border-gray-300 rounded text-sm py-2">
                    ...
                    @foreach($depots as $depot)
                        <option value="{{ $depot->id }}" data-bay-count="{{ $depot->tippingBays->count() }}">
                            {{ $depot->name }} ({{ $depot->tippingBays->count() }} bays)
                        </option>
                    @endforeach
                </select>
                @error('depot_id')
            ...
            <div>
                ...
                <input type="number" name="days" value="{{ old('days', 14) }}" min="1" max="90" required
        ...
        {{-- Bay Configuration Section (Hidden until depot selected) --}}
        <div id="bay-config-section" class="hidden">
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-6">
                ...
                <div id="bays-container" class="space-y-4">
                    {{-- Bays will be loaded here via JavaScript --}}
                </div>
            </div>
        </div>
        ...
        <div id="submit-section" class="hidden flex justify-end gap-3">
            <a href="{{ route('app.settings.dashboard') }}" class="px-6 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400">
                Cancel
            </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    depotSelect.addEventListener('change', function() {
        ...
        fetch(`{{ route('app.bay-slot-generation.get-bays') }}?depot_id=${depotId}`)
