                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center space-x-3">
                                    <div class="flex-shrink-0">
                                        <div class="w-10 h-10 bg-{{ $template->is_active ? 'green' : 'gray' }}-100 rounded-lg flex items-center justify-center">
                                            @if($template->file_type === 'csv')
                                                📄
                                            @elseif($template->file_type === 'xlsx')
                                                📊
                                            @elseif($template->file_type === 'xml')
                                                🔖
                                            @else
                                                📁
                                            @endif
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500">{{ $template->source_system }}</p>
                                    </div>
                                </div>
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-4 text-center">
                                <div class="bg-gray-50 rounded-lg p-3">
                                    <div class="text-lg font-semibold text-gray-900">{{ $template->files_processed ?? 0 }}</div>
                                    <div class="text-xs text-gray-500">Files Processed</div>
                                </div>
                                <div class="bg-gray-50 rounded-lg p-3">
                                    <div class="text-lg font-semibold text-gray-900">{{ count($template->column_mapping ?? []) }}</div>
                            ...
                            <div class="mb-4">
                                <div class="flex items-center text-sm text-gray-600">
                                    ...
                                    {{ $template->auto_process ? 'Auto-process files' : 'Manual review required' }}
                                </div>
                                @if($template->header_row)
                                    <div class="flex items-center text-sm text-gray-600">
                                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
                                        </svg>
                                        Header row: {{ $template->header_row }}
                                    </div>
                                @endif
                                <div class="flex items-center text-sm text-gray-600">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    Data starts: Row {{ $template->data_start_row }}
                                </div>
                            </div>
                            ...
                            <div class="flex justify-between">
                                <div class="flex space-x-2">
                                    <a href="{{ route('outbound.imports.templates.edit', $template) }}" 
                                ...
                                <div>
                                    @if($template->last_used_at)
                                        <span class="text-xs text-gray-500">
                                            Used {{ $template->last_used_at->diffForHumans() }}
                                        </span>
                                    @else
