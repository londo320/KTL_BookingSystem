...
@section('title', 'Create Import Template')
...
<div class="py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('outbound.imports.templates.store') }}" class="space-y-8">
            @csrf
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="source_system" 
                               name="source_system" 
                               value="{{ old('source_system') }}"
                               placeholder="e.g., SAP WMS, Manhattan, Custom ERP"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 @error('source_system') border-red-500 @enderror" 
                               required>
                        @error('source_system')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select id="file_type" 
                                name="file_type" 
                                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500 @error('file_type') border-red-500 @enderror" 
                                required
                            ...
                            <option value="txt" {{ old('file_type') === 'txt' ? 'selected' : '' }}>Text (TXT)</option>
                            <option value="xml" {{ old('file_type') === 'xml' ? 'selected' : '' }}>XML</option>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div id="csv-settings">
                            ...
                            <select id="delimiter" 
                                ...
                                <option value="," {{ old('delimiter') === ',' ? 'selected' : '' }}>Comma (,)</option>
                                <option value=";" {{ old('delimiter') === ';' ? 'selected' : '' }}>Semicolon (;)</option>
                                <option value="|" {{ old('delimiter') === '|' ? 'selected' : '' }}>Pipe (|)</option>
                                <option value="\t" {{ old('delimiter') === "\t" ? 'selected' : '' }}>Tab</option>
                            </select>
                        </div>
                        ...
                        <div id="text-qualifier-settings">
                            ...
                            <select id="text_qualifier" 
                                    name="text_qualifier" 
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                                <option value="\"" {{ old('text_qualifier') === '"' ? 'selected' : '' }}>Double Quote (")</option>
                                <option value="'" {{ old('text_qualifier') === "'" ? 'selected' : '' }}>Single Quote (')</option>
                                <option value="" {{ old('text_qualifier') === '' ? 'selected' : '' }}>None</option>
                            </select>
                        </div>
                        ...
                        <div id="encoding-settings" class="md:col-span-2">
                            ...
                            <select id="encoding" 
                                    name="encoding" 
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                                <option value="UTF-8" {{ old('encoding') === 'UTF-8' ? 'selected' : '' }}>UTF-8</option>
                                <option value="ISO-8859-1" {{ old('encoding') === 'ISO-8859-1' ? 'selected' : '' }}>ISO-8859-1 (Latin-1)</option>
                                <option value="Windows-1252" {{ old('encoding') === 'Windows-1252' ? 'selected' : '' }}>Windows-1252</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <select id="duplicate_handling" 
                                ...
                                required>
                            <option value="skip" {{ old('duplicate_handling') === 'skip' ? 'selected' : '' }}>Skip duplicates</option>
                            <option value="overwrite" {{ old('duplicate_handling') === 'overwrite' ? 'selected' : '' }}>Overwrite existing</option>
                            <option value="create_new" {{ old('duplicate_handling') === 'create_new' ? 'selected' : '' }}>Create new record</option>
                        </select>
