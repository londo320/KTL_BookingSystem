...
<div class="py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form action="{{ route('outbound.imports.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            ...
            <div class="bg-white rounded-lg shadow">
                ...
                <div class="p-6">
                    @if($templates->count() > 0)
                        <div class="space-y-6">
                            @foreach($templates as $system => $systemTemplates)
                                <div>
                                    <h3 class="text-sm font-medium text-gray-900 mb-3">{{ strtoupper(str_replace('_', ' ', $system)) }}</h3>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        @foreach($systemTemplates as $template)
                                            <label class="relative">
                                                <input type="radio" name="import_template_id" value="{{ $template->id }}" 
                                                       class="sr-only peer" 
                                                       {{ request('template') == $template->id ? 'checked' : '' }}
                                                       {{ $loop->parent->first && $loop->first && !request('template') ? 'checked' : '' }}>
