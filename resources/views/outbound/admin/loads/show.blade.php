...
@section('title', 'Load ' . $load->load_reference)
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-900">{{ $load->load_reference }}</h1>
                        @if($load->load_name)
                            <p class="text-gray-600 mt-1">{{ $load->load_name }}</p>
                        @endif
                    </div>
                ...
                <div class="flex space-x-3">
                    <a href="{{ route('outbound.loads.timing-analysis', $load) }}" 
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white rounded-lg shadow">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                            ...
                            <div class="text-center">
                                <div class="text-2xl font-bold text-purple-600">{{ $load->total_pallets }}</div>
                ...
                <div class="bg-white rounded-lg shadow">
                    <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                        <h2 class="text-lg font-semibold text-gray-900">Orders ({{ $load->orders->count() }})</h2>
                        @if($load->status === 'planning')
                            <a href="{{ route('outbound.loads.add-order', $load) }}" 
                                        ...
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4">
                                                <div class="text-sm text-gray-900">
                                                    {{ $order->customerAddress->address_line_1 }}
                                                </div>
                                                <div class="text-xs text-gray-500">
                                                    {{ $order->customerAddress->city }}, {{ $order->customerAddress->postcode }}
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">
                                                    {{ $order->expected_pallets }}p / {{ $order->expected_cases }}c / {{ $order->expected_units }}u
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 py-1 text-xs font-medium rounded-full
                                                    @switch($order->delivery_priority)
                                                        @case('standard')
                                                            bg-gray-100 text-gray-800
                                                            @break
                                                        @case('priority')
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                <a href="{{ route('outbound.orders.show', $order) }}" 
                ...
                <div class="bg-white rounded-lg shadow">
                    <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                        <h2 class="text-lg font-semibold text-gray-900">Collections ({{ $load->collections->count() }})</h2>
                        @if($load->status === 'planning')
                            <a href="{{ route('outbound.loads.add-collection', $load) }}" 
                    ...
                    <div class="overflow-x-auto">
                        @if($load->collections->count() > 0)
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($load->collections->sortBy('collection_sequence') as $collection)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900">{{ $collection->depot->name }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">{{ $collection->planned_collection_time->format('M j, Y H:i') }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">{{ $collection->collection_sequence ?? 'Not set' }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900">
                                                    {{ $collection->depot_pallets }}p / {{ $collection->depot_cases }}c / {{ $collection->depot_units }}u
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 py-1 text-xs font-medium rounded-full
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                                <a href="{{ route('outbound.collections.show', $collection) }}" 
                                                   class="text-blue-600 hover:text-blue-900">View</a>
                                            </td>
                                        </tr>
                                    @endforeach
                ...
                <div class="bg-white rounded-lg shadow">
                    ...
                    <div class="p-6 space-y-4">
                        <div>
                            ...
                            <dd class="text-sm text-gray-900">{{ $load->created_at->format('M j, Y H:i') }}</dd>
                        </div>
                        <div>
                            <dt class="text-sm font-medium text-gray-500">Source</dt>
                            <dd class="text-sm text-gray-900">{{ ucfirst(str_replace('_', ' ', $load->created_from)) }}</dd>
                        </div>
                        @if($load->plannedVehicle)
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Vehicle</dt>
                                <dd class="text-sm text-gray-900">{{ $load->plannedVehicle->registration }}</dd>
                            </div>
                        @endif
                        @if($load->assignedDriver)
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Driver</dt>
                                <dd class="text-sm text-gray-900">{{ $load->assignedDriver->name }}</dd>
                            </div>
                        @endif
                        @if($load->optimized_distance_km)
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Route Distance</dt>
                                <dd class="text-sm text-gray-900">{{ $load->optimized_distance_km }} km</dd>
                            </div>
                        @endif
                        @if($load->estimated_duration_minutes)
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Estimated Duration</dt>
                                <dd class="text-sm text-gray-900">{{ floor($load->estimated_duration_minutes / 60) }}h {{ $load->estimated_duration_minutes % 60 }}m</dd>
                            </div>
                        @endif
                        @if($load->notes)
                            <div>
                                <dt class="text-sm font-medium text-gray-500">Notes</dt>
                                <dd class="text-sm text-gray-900">{{ $load->notes }}</dd>
                            </div>
                        @endif
                    </div>
                </div>
