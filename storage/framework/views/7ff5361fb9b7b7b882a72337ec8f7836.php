...
<?php $component->withAttributes([]); ?>
    ...
    <script>
        ...
        async function showBayDetails(bayId) {
            try {
                const response = await fetch(`<?php echo e(route('app.depot-map.bay-status', '')); ?>/${bayId}`);
                if (response.ok) {
                    const data = await response.json();
        ...
        async function showLocationDetails(locationId) {
            ...
            try {
                const url = `<?php echo e(url($routePrefix . 'depot-map/location')); ?>/${locationId}?v=${Date.now()}`;
