...
<?php $component->withAttributes([]); ?>
  ...
  <div class="bg-white min-h-screen px-6 py-6">
    ...
    <?php if($bookingsNeedingPOs->count() > 0): ?>
    <div class="bg-red-50 p-4 rounded-lg border border-red-200 mb-4">
      <h3 class="text-base font-semibold text-red-900 mb-2">⚠️ Bookings Needing PO Details (<?php echo e($bookingsNeedingPOs->count()); ?>)</h3>
      ...
      <div class="bg-white rounded border border-red-200 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <?php $__currentLoopData = $bookingsNeedingPOs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $booking): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr class="hover:bg-gray-50">
              <td class="px-3 py-2 text-sm font-medium text-gray-900">#<?php echo e($booking['id']); ?></td>
              <td class="px-3 py-2 text-sm text-gray-700"><?php echo e($booking['reference']); ?></td>
              <td class="px-3 py-2 text-sm text-gray-700">
                <span class="font-mono text-xs bg-purple-100 text-purple-800 px-1 rounded">ID: <?php echo e($booking['customer_id']); ?></span>
                <span class="ml-1"><?php echo e($booking['customer_name']); ?></span>
              </td>
              <td class="px-3 py-2 text-sm text-gray-700"><?php echo e($booking['slot_time']); ?></td>
              <td class="px-3 py-2 text-sm text-gray-700"><?php echo e($booking['booking_type']); ?></td>
              <td class="px-3 py-2 text-sm text-gray-500"><?php echo e($booking['created_at']); ?></td>
            </tr>
    ...
    <?php if($accessibleCustomers->count() > 0): ?>
    <div class="bg-purple-50 p-4 rounded-lg border border-purple-200 mb-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
        <?php $__currentLoopData = $accessibleCustomers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $customer): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <div class="bg-white p-2 rounded border border-purple-200">
            <span class="font-mono font-semibold text-purple-900">ID: <?php echo e($customer->id); ?></span>
            <span class="text-sm text-gray-700 ml-2">- <?php echo e($customer->name); ?></span>
          </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
      </div>
    </div>
    ...
    <div class="bg-green-50 p-4 rounded-lg border border-green-200 mb-4">
      <div class="flex items-center justify-between">
        ...
        <a href="<?php echo e(route('app.bookings.download-generic-csv-template')); ?>"
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    let csvData = null;
    let recentBookings = <?php echo json_encode($recentBookings, 15, 512) ?>;
    ...
    if (analyzeBtn) {
      analyzeBtn.addEventListener('click', function() {
        ...
        fetch('<?php echo e(route("app.bookings.bulk-upload.process")); ?>?action=analyze', {
          method: 'POST',
          body: formData,
          headers: {
    ...
    if (processAllBtn) {
      processAllBtn.addEventListener('click', function() {
        ...
        fetch('<?php echo e(route("app.bookings.bulk-upload.process")); ?>?action=process', {
          method: 'POST',
          body: formData,
          headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    ...
    function displayResults(results) {
      resultsContainer.innerHTML = `
        ...
        <div class="mt-6">
          <a href="<?php echo e(route('app.bookings.index')); ?>" class="px-6 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 inline-block">
            View All Bookings
          </a>
        </div>
      `;
...
<?php endif; ?>
<?php /**PATH /Users/londo/Herd/test/resources/views/admin/bookings/bulk-upload.blade.php ENDPATH**/ ?>