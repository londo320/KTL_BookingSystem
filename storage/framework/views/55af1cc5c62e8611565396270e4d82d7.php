...
<?php $component->withAttributes([]); ?>
   ...
   <?php $__env->slot('header', null, []); ?> 
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2">
        ...
        <?php if (\Illuminate\Support\Facades\Blade::check('canFunction', 'bookings.fix-historical-departures')): ?>
        ...
        <?php if (\Illuminate\Support\Facades\Blade::check('canFunction', 'customer-behavior.view')): ?>
        ...
        <?php if (\Illuminate\Support\Facades\Blade::check('canFunction', 'bookings.create')): ?>
        <a href="<?php echo e(route('app.bookings.create')); ?>"
  ...
  <div class="py-6 max-w-7xl mx-auto">
      ...
      <div class="p-4 border-b border-gray-200">
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-4 items-center">
          ...
          <div class="xl:col-span-1">
            <form method="GET" action="<?php echo e(route('app.bookings.index')); ?>" class="flex gap-2">
              ...
              <?php if(request('search')): ?>
                <a href="<?php echo e(route('app.bookings.index', request()->except(['search', 'page']))); ?>"
          ...
          <?php if (\Illuminate\Support\Facades\Blade::check('hasAnyFunction', ['bookings.export.pdf', 'bookings.export.excel', 'bookings.export.csv'])): ?>
          <div class="xl:col-span-1 flex justify-end gap-1">
            <div class="flex gap-1">
              <?php if (\Illuminate\Support\Facades\Blade::check('canFunction', 'bookings.export.pdf')): ?>
              <a href="<?php echo e(route('app.bookings.export.pdf', request()->query())); ?>" 
                 class="px-2 py-1 bg-red-600 text-white rounded hover:bg-red-700 text-xs" target="_blank" title="Export PDF">
                📄 PDF
              </a>
              <?php endif; ?>
              
              <?php if (\Illuminate\Support\Facades\Blade::check('canFunction', 'bookings.export.excel')): ?>
              <a href="<?php echo e(route('app.bookings.export.excel', request()->query())); ?>" 
                 class="px-2 py-1 bg-green-600 text-white rounded hover:bg-green-700 text-xs" title="Export Excel">
                📊 Excel
              </a>
              <?php endif; ?>
              
              <?php if (\Illuminate\Support\Facades\Blade::check('canFunction', 'bookings.export.csv')): ?>
              <a href="<?php echo e(route('app.bookings.export.csv', request()->query())); ?>" 
      ...
      <div class="p-4">
        <div class="flex flex-wrap items-center gap-4">
          ...
          <div class="flex gap-2 items-center">
            ...
            <a href="<?php echo e(route('app.bookings.index', array_merge(request()->except(['status', 'page']), ['status' => 'outstanding']))); ?>" 
            ...
            <a href="<?php echo e(route('app.bookings.index', array_merge(request()->except(['status', 'page']), ['status' => 'completed']))); ?>" 
            ...
            <a href="<?php echo e(route('app.bookings.index', array_merge(request()->except(['status', 'page']), ['status' => 'all']))); ?>" 
          ...
          <div class="flex flex-wrap gap-2 items-center">
            ...
            <a href="<?php echo e(route('app.bookings.index', array_merge(request()->except(['filter', 'page']), ['filter' => 'yesterday']))); ?>" 
            ...
            <a href="<?php echo e(route('app.bookings.index', array_merge(request()->except(['filter', 'page']), ['filter' => 'today']))); ?>" 
            ...
            <a href="<?php echo e(route('app.bookings.index', array_merge(request()->except(['filter', 'page']), ['filter' => 'tomorrow']))); ?>" 
            ...
            <a href="<?php echo e(route('app.bookings.index', array_merge(request()->except(['filter', 'page']), ['filter' => 'last_week']))); ?>" 
            ...
            <a href="<?php echo e(route('app.bookings.index', array_merge(request()->except(['filter', 'page']), ['filter' => 'this_week']))); ?>" 
            ...
            <a href="<?php echo e(route('app.bookings.index', array_merge(request()->except(['filter', 'page']), ['filter' => 'next_week']))); ?>" 
      ...
      <div id="advanced-filters-content" class="hidden border-t border-gray-200">
        <form method="GET" class="p-4">
          ...
          <div class="mt-4 flex justify-end gap-2">
            ...
            <a href="<?php echo e(route('app.bookings.index')); ?>" class="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 text-sm font-medium">
              🔄 Reset All
            </a>
          </div>
        </form>
    ...
    <table class="min-w-full bg-white shadow rounded overflow-hidden text-sm">
      ...
      <tbody>
  <?php $__currentLoopData = $bookings->groupBy(fn($b) => $b->slot?->depot?->name ?? 'Unknown Depot'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $depotName => $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <tr><td colspan="11" class="bg-gray-200 font-semibold px-4 py-2">Depot: <?php echo e($depotName); ?></td></tr>
    <?php $__currentLoopData = $group->sortBy(fn($b) => $b->slot?->start_at ?? now()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $booking): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <?php
        $isFactory = isset($booking->type) && $booking->type === 'factory';
      ?>
      <tr class="border-t hover:bg-gray-50 
        <?php if($isFactory): ?> bg-purple-50 border-l-4 border-purple-400
        <?php elseif($booking->cancelled_at): ?> bg-red-50 border-red-200 
        ...
        <td class="px-4 py-2 align-top">
          <div class="flex items-center">
            ...
            <?php endif; ?>
            <div class="font-mono text-sm font-semibold <?php echo e($isFactory ? 'text-purple-600' : 'text-blue-600'); ?>">
              <?php if($isFactory): ?>
            ...
            <?php if($isFactory): ?>
              <div class="text-xs bg-purple-100 text-purple-700 px-1 rounded mt-1">Factory</div>
            <?php endif; ?>
          </div>
        </td>
                ...
                <div class="text-sm">
                  <?php if($arrivedAt): ?>
                    <div class="text-gray-700">
                      Arrived: <?php echo e($arrivedAt->format('d-M H:i')); ?>

                    </div>
                    <?php if(!$completedAt): ?>
                      <div class="<?php echo e($isOverdue ? 'text-red-600 font-semibold' : 'text-blue-600'); ?> text-xs">
                        On site: <?php echo e(floor($minutesOnSite / 60)); ?>h <?php echo e($minutesOnSite % 60); ?>m
                        <?php if($isOverdue): ?>
                          <span class="bg-red-100 px-1 rounded">OVERDUE</span>
                        <?php endif; ?>
                      </div>
                    <?php else: ?>
                      <div class="text-green-600 text-xs">
                        Completed: <?php echo e($completedAt->format('d-M H:i')); ?>

                      </div>
                    <?php endif; ?>
                  <?php else: ?>
                    <div class="text-gray-500 text-sm">Not arrived</div>
                  <?php endif; ?>
                </div>
              <?php else: ?>
        ...
        <td class="px-4 py-2 align-top">
          <div class="text-sm font-medium <?php echo e($isFactory ? 'text-purple-600' : ''); ?>"><?php echo e(optional($booking->bookingType)->name ?? '-'); ?></div>
        ...
        <td class="px-4 py-2 align-top">
          <?php if($booking->carrier): ?>
            <div class="text-sm font-medium"><?php echo e($booking->carrier->name); ?></div>
          <?php elseif($booking->carrier_company): ?>
            <div class="text-sm text-gray-600"><?php echo e($booking->carrier_company); ?></div>
          <?php else: ?>
        ...
        <td class="px-4 py-2 align-top">
          ...
          ?>
          <?php if($expectedBay): ?>
            <div class="text-sm font-medium text-green-700"><?php echo e($expectedBay->name); ?></div>
            <?php if($expectedBay->code): ?>
              <div class="text-xs text-gray-500"><?php echo e($expectedBay->code); ?></div>
            <?php endif; ?>
          <?php elseif($booking->slot?->tippingBay): ?>
            <div class="text-sm text-gray-600"><?php echo e($booking->slot->tippingBay->name); ?></div>
        ...
        <td class="px-4 py-2 align-top space-y-1">
          ...
          <?php else: ?>
            ...
            <?php if($isFactory && $booking->trailer_registration): ?>
              <div class="text-xs text-gray-600">🚚 <?php echo e($booking->trailer_registration); ?></div>
            <?php endif; ?>
          ...
          <?php if($booking->arrived_at && (!$booking->departed_at || $isFactory)): ?>
            ...
            <div class="space-y-1">
                ...
                <div class="text-xs text-gray-600">
                  <?php if($isEmptyTrailer): ?>
                    ...
                    <?php if($movement->current_status === 'awaiting_collection' && $movement->tippingLocation): ?>
                      <div class="text-green-700">📦 <?php echo e($movement->tippingLocation->name); ?> (Parking Area)</div>
              ...
              <?php if($booking->tipping_status && $booking->tipping_status !== 'departed'): ?>
                <?php $canTakeAction = $booking->slot->depot_id == $defaultDepotId; ?>
                <?php if($canTakeAction): ?>
                  <?php if(isset($booking->type) && $booking->type === 'factory'): ?>
                    <a href="<?php echo e(route('app.factory-booking-workflow.show', $booking->original_factory_booking)); ?>" 
                ...
                <?php else: ?>
                  <span class="text-xs text-gray-400 cursor-not-allowed block" 
                        title="Actions only available for your default depot">
                    <?php if(isset($booking->type) && $booking->type === 'factory'): ?> Factory Workflow <?php else: ?> Manage <?php endif; ?> →
                  </span>
                <?php endif; ?>
              <?php endif; ?>
            </div>
        ...
        <td class="px-4 py-2 align-top space-y-1">
          <div class="flex flex-col space-y-1">
            ...
            <?php if(isset($booking->type) && $booking->type === 'factory'): ?>
              <a href="<?php echo e(route('app.factory-bookings.show', $booking->original_factory_booking)); ?>"
            ...
            <?php if(!$isCancelled): ?>
              ...
              <?php if($canTakeAction): ?>
                <?php if(isset($booking->type) && $booking->type === 'factory'): ?>
                  <a href="<?php echo e(route('app.factory-bookings.edit', $booking->original_factory_booking)); ?>"
  ...
  <div id="quickTrailerCollectionModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
      <div class="mt-3">
        ...
        <form action="<?php echo e(route('app.empty-unit-collection.process')); ?>" method="POST">
          <?php echo csrf_field(); ?>
  ...
  <script>
    ...
    function openTrailerCollectionModal() {
      ...
      window.open('<?php echo e(route("app.empty-unit-collection")); ?>', '_blank', 'width=1200,height=800,scrollbars=yes,resizable=yes');
    }
...
<?php endif; ?>
<?php /**PATH /Users/londo/Herd/test/resources/views/warehouse/bookings/index.blade.php ENDPATH**/ ?>